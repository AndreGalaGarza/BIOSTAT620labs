...
cv_states <- cv_states |> mutate(naive_CFR = round((cv_states$deaths / cv_states$cases),2))
...
cv_states_cfr <- cv_states %>%
  filter(cv_states$naive_CFR < 1)
...
cv_states |> filter(state=="Florida") |> 
  plot_ly(x = ~date, y = ~new_cases, type = "scatter", mode = "lines", hoverinfo = 'text') |> 
  add_lines(x = ~date, y = ~new_deaths, type = "scatter", mode = "lines", hoverinfo = 'text') 
```

The peak of cases appears to be in February 2022, which is four months after the peak of deaths in October 2021.
