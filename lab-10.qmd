...
cv_states <- read_csv('https://raw.githubusercontent.com/nytimes/covid-19-data/refs/heads/master/us-states.csv')

### FINISH THE CODE HERE ###
# load state population data
state_pops <- read_csv('https://raw.githubusercontent.com/COVID19Tracking/associated-data/refs/heads/master/us_census_data/us_census_2018_population_estimates_states.csv')
...
### COMPLETE CODE HERE
p1 <- ggplot(cv_states, aes(date, new_cases)) +
               geom_point()
ggplotly(p1)

p2 <- ggplot(cv_states, aes(date, new_deaths)) +
               geom_point()
ggplotly(p2)
```
...
cv_states <- cv_states |> mutate(naive_CFR = round((cv_states$new_deaths / cv_states$new_cases),2))
...
p <- ggplot(cv_states, aes(x = date, y=naive_CFR, group=state),) + geom_line()
ggplotly(p)

### FINISH CODE HERE
# Line chart for Florida showing new_cases and new_deaths together (two lines)
cv_states |> filter(state=="Florida") |> 
  plot_ly(x = ~date, y = ~newper100k, type = "scatter", mode = "lines", hoverinfo = 'text') |> 
  add_lines(x = ~date, y = ~deathsper100k, type = "scatter", mode = "lines", hoverinfo = 'text',
            text = ~paste( paste(state, ":", sep=""), paste(" Cases per 100k: ", per100k, sep="") , 
                         paste(" Deaths per 100k: ", deathsper100k, sep=""), sep = "<br>"))
```

After hovering over the cases and deaths per 100k people in Florida over time with hoverinfo, the peak of cases appears to be in February 2022 (at 726.8 cases per 100k), which is four months after the peak of deaths in October 2021 (at 9.3 deaths per 100k).
...
From the heatmap of new_cases, the state of California stands out as receiving the most new cases (close to 100k) in January 2022.

From the heatmap of newper100k, the state of Rhode Island appeared to have the most cases (nearly 600) per 100k relative to its population.

Finally, the heatmap that shows dates every 2 weeks shows how e.g. Louisiana hit its peak number of cases in mid-August 2021, before Mississippi hit its peak in late August, South Deakota and Texas peaked in early September, and Alabama peaked the latest in October 2021.
...
# Create hover text to include state name, new cases per 100k, cases, and deaths
cv_per100$hover <- with(cv_per100, paste(state.name, "<br>New cases per 100k", newper100k, "<br>Cases:", cases, "<br>Deaths:", deaths))
...
### Plot together 
#ggplotly(fig_pick.date, fig_Today, nrows = 2, margin = .05)
subplot(fig_pick.date, fig_Today)
```

From this map, we can see that the cases as of 2021-10-15 are most prevalent in the states of Alaska, Montana, and Wyoming. However, the new cases as of today (2026-03-27) have nearly disappeared completely, to the point where the chart is almost completely white and it is difficult to discern any differences in case counts without hovering over each individual state.