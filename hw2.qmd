...
```
Note that for some of the records in the dataset, the values of "gasstove" and "smoke" were not binary (i.e. either 0 or 1). For these cases, the "gasstove" and "smoke" categorical variables were rounded to either 0 or 1 depending on whether the original value was greater than or equal to 0.5.
...
chs_imp <- chs_imp %>%
  mutate(
    shs_bin = case_when(
      is.na(.data[[shs_var]]) ~ NA_real_,
      .data[[shs_var]] %in% c(1, "1", "Y", "Yes", "YES", TRUE) ~ 1,
      .data[[shs_var]] %in% c(0, "0", "N", "No",  "NO",  FALSE) ~ 0,
      suppressWarnings(!is.na(as.numeric(as.character(.data[[shs_var]])))) ~
        if_else(
          as.numeric(as.character(.data[[shs_var]])) >= 0.5,
          1, 0
        ),
      TRUE ~ NA_real_
    ),
    gas_bin = case_when(
      is.na(.data[[gas_var]]) ~ NA_real_,
      .data[[gas_var]] %in% c(1, "1", "Y", "Yes", "YES", TRUE) ~ 1,
      .data[[gas_var]] %in% c(0, "0", "N", "No",  "NO",  FALSE) ~ 0,
      suppressWarnings(!is.na(as.numeric(as.character(.data[[gas_var]])))) ~
        if_else(
          as.numeric(as.character(.data[[gas_var]])) >= 0.5,
          1, 0
        ),
      TRUE ~ NA_real_
    ),
...
```
From these histograms, we see that FEV follows a unimodal distribution centered around ~1900-2200 ml. It is roughly centered around the mean without being clearly left-tailed or right-tailed. The BMI distribution is clearly right-skewed, with most values clustered around ~14-20 followed by a long tail. Finally, the distribution of PM2.5 is not smooth, but instead contains one "spike" of either 100 or 200 records per community, since PM2.5 is measured at the community-level for 12 communities across southern California. In other words, the community-level analysis is the reason why the same measurements are being repeated either 100 or 200 times per community.
...
df_plot <- df1 %>%
  mutate(
    bmi_resid = resid(lm(bmi ~ male + height + agepft, data = df1)),
    fev_resid = resid(lm(fev ~ male + height + agepft, data = df1))
  )

# Put both relationships in one panel by standardizing axes:
# - Crude: (bmi, fev)
# - Adjusted (AV): (bmi_resid, fev_resid)
long_plot <- bind_rows(
  df_plot %>%
    transmute(x = bmi, y = fev, type = "Crude (FEV vs BMI)"),
  df_plot %>%
    transmute(x = bmi_resid, y = fev_resid, type = "Adjusted (added-variable residuals)")
)

ggplot(long_plot, aes(x = x, y = y, color = type)) +
  geom_point(alpha = 0.25) +
  geom_smooth(method = "lm", se = FALSE, linewidth = 1) +
  facet_wrap(~type, scales = "free") +
  labs(
    title = "Crude vs adjusted association between BMI and FEV",
    x = NULL,
    y = NULL,
    color = NULL
  ) +
...
The side-by-side plot above contrasts the raw relationship with the “net” relationship after accounting for key physiology covariates. On the right, the crude scatter shows a steep upward regression line, reflecting that heavier individuals tend to have higher observed FEV in this sample. On the left, the added-variable (residual–residual) plot isolates what BMI explains beyond sex, height, and age: the fitted line remains upward but is noticeably flatter amid substantial scatter around zero, indicating that much of the apparent BMI–FEV trend is shared with overall body size while a smaller, still positive BMI signal persists once those factors are removed.
...
df2_sum <- df2 %>%
  group_by(smoke_gas_exposure) %>%
  summarise(
    raw_mean = mean(.data[["fev"]], na.rm = TRUE),
    raw_sd   = sd(.data[["fev"]], na.rm = TRUE),
    n = n(),
    .groups = "drop"
  ) %>%
  mutate(raw_se = raw_sd / sqrt(n),
         raw_ci_lo = raw_mean - 1.96 * raw_se,
         raw_ci_hi = raw_mean + 1.96 * raw_se)

adj_df <- as.data.frame(adj_tab) %>%
  mutate(
    adj_ci_lo = emmean - qt(0.975, df) * SE,
    adj_ci_hi = emmean + qt(0.975, df) * SE
  ) %>%
  transmute(
    smoke_gas_exposure,
    adj_mean = emmean,
...
means_both <- df2_sum %>%
  select(smoke_gas_exposure, raw_mean, raw_ci_lo, raw_ci_hi) %>%
  left_join(adj_df, by = "smoke_gas_exposure") %>%
  tidyr::pivot_longer(
    cols = c(raw_mean, adj_mean),
    names_to = "type",
    values_to = "mean"
  ) %>%
  mutate(
    type = recode(type, raw_mean = "Raw", adj_mean = "Adjusted"),
    ci_lo = if_else(type == "Raw", raw_ci_lo, adj_ci_lo),
    ci_hi = if_else(type == "Raw", raw_ci_hi, adj_ci_hi)
  )

ggplot(means_both, aes(x = smoke_gas_exposure, y = mean, group = type)) +
  geom_point(aes(shape = type), position = position_dodge(width = 0.35), size = 2) +
  geom_errorbar(
    aes(ymin = ci_lo, ymax = ci_hi, linetype = type),
    position = position_dodge(width = 0.35),
    width = 0.12
  ) +
  labs(
    title = "Raw vs adjusted mean FEV by smoke/gas exposure",
    subtitle = "Adjusted means from m2_adj via emmeans; CIs shown",
    x = "Exposure category",
    y = "Mean FEV",
    shape = NULL,
    linetype = NULL
...
The grouped scatterplot above compares simple (raw) mean FEV by smoke/gas exposure group to regression-adjusted mean FEV after controlling for sex, height, and age, with 95% confidence intervals. Overall, the raw means are fairly similar across categories and the adjusted means shift only slightly (most noticeably for the secondhand-smoke-only group), but in both cases the confidence intervals overlap substantially. This suggests differences in average FEV across exposure groups are modest relative to within-group variability, and covariate adjustment does not produce a dramatic separation between categories.
...
df3_plot <- df3 %>%
  mutate(
    pm25_resid = resid(lm(pm25_mass ~ male + height + agepft, data = df3)),
    fev_resid  = resid(lm(fev ~ male + height + agepft, data = df3))
  )

ggplot(df3_plot, aes(x = pm25_resid, y = fev_resid)) +
  geom_point(alpha = 0.3) +
  geom_smooth(method = "lm", se = FALSE) +
  labs(
    title = "Adjusted association (added-variable plot): PM2.5 vs FEV",
    subtitle = "Residual(FEV | male + height + agepft) vs Residual(PM2.5 | male + height + agepft)",
    x = "PM2.5 residualized",
    y = "FEV residualized"
  ) +
  theme_minimal()
```
This added-variable (partial regression) plot shows the relationship between PM2.5 and FEV after removing the effects of sex, height, and age from both variables: the x-axis is the residualized PM2.5 (variation not explained by male/height/agepft) and the y-axis is the residualized FEV. Just like the plot of the original variables, the fitted line is essentially flat with only a very slight downward tilt, indicating that once those physiological covariates are accounted for, PM2.5 still has little independent linear association with FEV in this dataset. The wide scatter around the line reinforces that the remaining PM2.5 signal is small relative to the residual variability in FEV, consistent with a weak and possibly non-significant adjusted effect.
